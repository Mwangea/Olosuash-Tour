...
export const BookingModal = ({ 
  ...
  const [paymentMethod, setPaymentMethod] = useState<'online' | 'whatsapp'>('whatsapp');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const bookingData = {
        ...
        user_id: user?.id,
        phone: user?.phone || whatsappNumber, // Fixed: Use user's phone or whatsapp number
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      className="mr-2"
                    />
                    <label htmlFor="whatsapp">Pay via WhatsApp</label>
                  </div>
                </div>
                {paymentMethod === 'online' && (
                  <div className="mt-2 p-2 bg-yellow-50 text-yellow-700 text-sm rounded-md">
                    Online payment is coming soon. Please use WhatsApp payment for now.
                  </div>
                )}
              </div>
